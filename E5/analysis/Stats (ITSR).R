...
subj_avgs = group_by(data, subject, pitch, range) %>% 
  summarize(residual = mean(residual, na.rm=T))
model = lm(residual ~ 1 + poly(pitch, 5), filter(subj_avgs, range == 0))
summary(model)
anova_stats(model)
...
subj_avgs = group_by(data, subject, pitch, range) %>% 
  summarize(residual = mean(residual, na.rm=T))
model = lm(residual ~ 1 + poly(pitch, 5), filter(subj_avgs, range == 1))
summary(model)
anova_stats(model)

# Pitch x Octave Range
data$pitch = as.numeric(data$pitch)
data$type = factor(data$type, ordered=F)
subj_avgs = group_by(data, subject, pitch, range) %>% 
  summarize(residual = mean(residual, na.rm=T))
model = lm(residual ~ 1 + poly(pitch, 1) * range, subj_avgs)
summary(model)
anova_stats(model)



### RAW RATINGS BY OCTAVE RANGE ###

subj_avgs = group_by(data, subject, range) %>% 
  summarize(response = mean(response, na.rm=T))
t.test(x=subj_avgs[subj_avgs$range==0,]$response, 
       y=subj_avgs[subj_avgs$range==1,]$response, 
