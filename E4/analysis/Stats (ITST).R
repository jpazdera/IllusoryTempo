...
# Linear Slope (*)
# t(76)=2.12, p=.037, d=0.242, M=6.82, CI=[0.42, 13.21]
ggqqplot(fits$b1)
t.test(fits$b1, mu=0, conf.level=.95, alternative="two.sided")
mean(fits$b1) / sd(fits$b1)
# Quadratic Slope (***)
# t(76)=-4.87, p<.001, d=0.555, M=-14.39, CI=[-20.28, -8.50]
ggqqplot(fits$b2)
...
# Effect of Tone Type on Slopes (n.s.)
# F(2, 75)=2.77, p=.069, pes=0.282
diff <- fits[fits$type=='p', c('b1', 'b2')] - fits[fits$type=='a', c('b1', 'b2')]
T2 <- HotellingsT2(diff, mu=rep(0, 2), na.action=drop, test='f')
print(T2)
T2$parameter[['df1']] * T2$statistic[[1]] / (T2$parameter[['df1']] * T2$statistic[[1]] + T2$parameter[['df2']])

# Intercept (Main effect of tone type) (**)
# t(76)=-3.38, p=.001, d=0.720, diff=-1.60, CIdiff=[-2.54, -0.66]
