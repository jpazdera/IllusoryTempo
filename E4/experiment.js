...
var tone_types = jsPsych.randomization.repeat(['p', 'a'], 1);

jsPsych.data.addProperties({
    subject: subjectID,
    first_type: tone_types[0],
    experiment: 'ITST',
    code_version: 'v1.0'
});
...
$.getJSON(`schedules/session${subjectID}.json`).done(function (schedule) {
    ...
    var debrief = {
        ...
        stimulus: '<h1>About This Study</h1>' +
            '<h3>Principal Investigator: Dr. Laurel J. Trainor (ltj@mcmaster.ca)</h3>' +
            '<h3>Researcher: Jesse K. Pazdera, B.Sc. (pazderaj@mcmaster.ca)</h3>' +
            '<p style="text-align: justify; text-indent: 35px; max-width: 750px;">In this study, you listened to sequences of ' +
            'metronome clicks and repeating tones and rated the speed of each tone sequence. The metronome always ' +
            'played at the same rate, while the tones varied in both their pitch (how high or low they were) and ' +
            'their tempo (how quickly they repeated). Some tones were played slower than the metronome, some were ' +
            'played faster, and others were played at precisely the same rate. We are interested in whether the ' +
            'pitch of a sound sequence can make it appear to be faster or slower than it truly is. Previous ' +
            'research suggests that people perceive higher-pitched speech and music to be faster than lower-pitched ' +
            'speech and music, even when both are played at the same rate. However, most experiments have only ' +
            'compared one higher octave to one lower octave, making it difficult to understand how perceived tempo ' +
            'changes across the broader range of sounds we might hear. Our study seeks to answer this question by ' +
            'testing the perceived tempo of sounds across six full octaves (for comparison, most modern pianos span ' +
            'seven).</p>' +
            '<p style="text-align: justify; text-indent: 35px; max-width: 750px;">Additionally, we are interested in whether the ' +
            'effect of pitch differs depending on the type of sound you are listening to. On some trials you ' +
            'listened to piano tones, while on others you listened to artificially constructed tones. This test ' +
            'will help us to identify whether the natural irregularities of piano tones lead to different outcomes ' +
            'than do carefully controlled artificial tones.</p>' +
            '<p style="text-align: justify; text-indent: 35px; max-width: 750px;">In total, you heard two types of tones played at ' +
            'six different pitches and 15 different rates. Your responses will help us to map out the illusory ' +
            'tempo effect across a wide range of sounds.</p>' +
            '<h3 style="max-width: 750px;">We would like to thank you again for participating in our study. If you would like to learn more ' +
            'about this topic, please contact Jesse Pazdera at the email listed above.</h3>' 
    };
    ...
    if (tone_types[0] == 'p') {
        var timeline_vars = [
            { stimulus: 'stimuli/practice_sequence_741p.wav', ioi: 741 },
            { stimulus: 'stimuli/practice_sequence_550p.wav', ioi: 550 },
            { stimulus: 'stimuli/practice_sequence_407p.wav', ioi: 407 },
        ];
    } else {
        var timeline_vars = [
            { stimulus: 'stimuli/practice_sequence_741a.wav', ioi: 741 },
            { stimulus: 'stimuli/practice_sequence_550a.wav', ioi: 550 },
            { stimulus: 'stimuli/practice_sequence_407a.wav', ioi: 407 },
        ];
    }
    ...
    var practice_strings = ['sequence_741', 'sequence_550', 'sequence_407'];
    ...
    for (i in practice_strings) {
        ...
        audio_files.add(`stimuli/practice_${s}p.wav`);
        audio_files.add(`stimuli/practice_${s}a.wav`);
    }
    ...
    for (block = 0; block < n_blocks; block++) {
        for (trial = 0; trial < trials_per_block; trial++) {
            audio_files.add(`stimuli/sequence_${tone_types[block % 2]}_${schedule[block][trial][0]}_${schedule[block][trial][1]}_${schedule[block][trial][2]}.wav`)
            // Metronome
            timeline.push({
                    type: 'audio-multiple-response',
                    data: { 
                        event: 'metronome',
                        type: `${tone_types[block % 2]}`,
            ...
            timeline.push({
                ...
                stimulus: `stimuli/sequence_${tone_types[block % 2]}_${schedule[block][trial][0]}_${schedule[block][trial][1]}_${schedule[block][trial][2]}.wav`,
                response_ends_trial: false,
                trial_ends_after_audio: true
            });
            // Response Slider
